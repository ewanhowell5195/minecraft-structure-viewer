...
- Version comparison: the Compare panel picks a second game version with its own
  ...
  the compared version gained, changed (compared on content, so DataVersion
  bumps and re-serialised files don't count) and dropped, each with its count. An empty list has no tab, and two versions
